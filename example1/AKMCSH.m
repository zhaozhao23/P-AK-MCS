...
while (1)
    ...
    H=abs(log(sqrt(2*pi)*sqrt(s2)+1/2).*(normcdf((2*sqrt(s2)-mu)./sqrt(s2))-normcdf((-2*sqrt(s2)-mu)./sqrt(s2)))-...
        ((2*sqrt(s2)-mu)/2.*normpdf((2*sqrt(s2)-mu)./sqrt(s2))+(2*sqrt(s2)+mu)/2.*normpdf((-2*sqrt(s2)-mu)./sqrt(s2))));
    [a,b]=max(H);
    if a<0.1;break;end
    x_new=x(b,:);
    y_new=g(x_new);
